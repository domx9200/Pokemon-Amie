...
#                 Further Adapted from rigbycwts's script v1.5
################################################################################
=begin
...
#Estas alteracoes nao requerem outras scripts
################################################################################
=begin
...
EATABLE_ITEMS = [
  ...
  :ROWAPBERRY
=end
AMIEPOKEMON = 111 # Variable number to store the main Amie/Refresh Pokemon
AMIEINDEX = 112 # Variable number to store the main Amie/Refresh Pokemon Party Index
...
class PokeBattle_Pokemon
  ...
  def feedAmie(food)
    ...
    return if getFullnessLevel==5 # reduntant
      ...
      if @sprites["feedshow"]!=nil
        if !@sprites["feedshow"].disposed?
          if ($mouse.leftPress?)&&(!$mouse.overPixel?(@sprites["feed"]))&&($mouse.overPixel?(@sprites["feedshow"]))
            if (@mouse_x!=nil)
################################################################################
#Fix Script: Fixed error when feeding a Pokemon
#Alteracoes Para o Jogo Pokemon Chronicles, Codigo Original nos Comentarios a Verde
#Estas alteracoes nao requerem outras scripts
################################################################################
              return if !@sprites["food0"]
################################################################################
              dx=$mouse.x-@mouse_x
